...
serve(async (req) => {
  ...
  try {
    ...
    const listings = (startupsToGoLive || [])
      .filter((s: any) => {
        // Paid startups (premium/featured) go live on the launch day the maker
        // chose — but ONLY if payment has been confirmed.
        if (s.plan === 'premium' || s.plan === 'featured') {
          if (s.payment_status !== 'paid') {
            console.log(`Skipping unpaid ${s.plan} startup ${s.title} (payment_status=${s.payment_status})`)
            return false
          }
          // Same-day paid launches are flipped live immediately by the Stripe
          // webhook, so a not-live paid row here is either scheduled for a
          // future day (hold it) or due today/overdue (publish it). On the day
          // itself, wait for the 8 AM PST sweep so it matches the time we
          // promised the maker; overdue rows publish right away.
          if (s.launch_date && s.launch_date > todayPst) {
            console.log(`Holding scheduled ${s.plan} startup ${s.title} until ${s.launch_date}`)
            return false
          }
          if (s.launch_date === todayPst && currentHour < 8) {
            console.log(`Holding ${s.plan} startup ${s.title} until 8 AM PST on launch day (${currentHour} PST)`)
            return false
          }
...
async function sendLiveNotification(listing: any, blogUrl: string | null = null): Promise<boolean> {
  try {
    ...
    // keep reminding them they can still claim the free DR 44+ link equity.
    ...
    <div style="padding: 30px;">
      ...
      <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; border-radius: 0 8px 8px 0; padding: 20px; margin: 25px 0;">
        ...
        <p style="margin: 0 0 15px 0; color: #92400e; font-size: 14px; font-weight: bold;">
          Top 3 products earn a permanent badge + a dofollow backlink from our 44+ DR site.
        </p>
      ...
      <div style="background-color: #eff6ff; border-left: 4px solid #3b82f6; border-radius: 0 8px 8px 0; padding: 20px; margin: 25px 0;">
        <h3 style="margin: 0 0 8px 0; color: #1e40af; font-size: 16px;">Claim your free DR 44+ backlink</h3>
        <p style="margin: 0 0 14px 0; color: #1e3a8a; font-size: 14px; line-height: 1.6;">
          You launched without a do-follow SubmitHunt backlink — so you're not yet getting the DR 44+ link equity that lifts your own SEO. Add our badge to your homepage or footer (keep it do-follow) and it's yours, free.
        </p>
      ...
      <div style="background: #1a1a1a; border-radius: 8px; padding: 28px; margin: 25px 0;">
        ...
        <p style="margin: 0 0 20px 0; color: #ccc; font-size: 14px; line-height: 1.6;">
          Launch day gets you the spike — the second week is where steady traffic compounds. Premium doubles your homepage run and includes a guaranteed dofollow backlink from our DR 44+ site (founders routinely pay $50–200 for one of those elsewhere).
        </p>
          ...
          <tr>
            <td style="padding: 8px 0; border-bottom: 1px solid #333;">
              <table cellpadding="0" cellspacing="0"><tr>
                ...
                <td style="color: #e5e5e5; font-size: 14px;">Permanent dofollow backlink (44+ DR)</td>
              </tr></table>
            </td>
          </tr>
          <tr>
      ...
      <div style="background: linear-gradient(135deg, #7c3aed 0%, #5b21b6 100%); border-radius: 8px; padding: 25px; margin: 25px 0;">
        ...
        <p style="margin: 0 0 12px 0; color: #fff; font-size: 14px; line-height: 1.6; opacity: 0.95;">
          ${listing.title} is running with featured placement in the feed — gradient-border card, prime visibility to every visitor for the next 7 days. Your guaranteed dofollow backlink (DR 44+) goes live within 24 hours.
        </p>
      ...
      <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 8px; padding: 25px; margin: 25px 0;">
        ...
        <p style="margin: 0 0 12px 0; color: #fff; font-size: 14px; line-height: 1.6; opacity: 0.95;">
          ${listing.title} gets priority placement and stays on the homepage for 14 days — double the standard run. Your guaranteed dofollow backlink (DR 44+) goes live within 24 hours, and we'll feature you in our next newsletter to 2,000+ subscribers.
        </p>
...
${showBacklinkReminder ? `CLAIM YOUR FREE DR 44+ BACKLINK
You launched without a do-follow SubmitHunt backlink, so you're missing the DR 44+ link equity that lifts your own SEO. Add our badge to your homepage or footer (keep it do-follow) and it's yours, free.
...
- Permanent dofollow backlink (44+ DR)
